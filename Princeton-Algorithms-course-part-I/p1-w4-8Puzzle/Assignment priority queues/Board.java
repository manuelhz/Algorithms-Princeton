...
public class Board {
    ...
    public Iterable<Board> neighbors() {
        ...
        if(y == 0) top = null;
        else top = new char[n][n];

        if(y == n - 1) bottom = null;
        else bottom = new char[n][n];

        if(x == 0) left = null;
        else left = new char[n][n];

        if(x == n - 1) right = null;
        else right = new char[n][n];
    ...
    private static void swap(char[][] a, int i, int j, int k, int l) {
        char copy = a[i][j];
        a[i][j] = a[k][l];
        a[k][l] = copy;
    }
